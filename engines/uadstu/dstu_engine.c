...
static const char *engine_dstu_name =
    "Reference implementation of DSTU engine";
...
static int dstu_pkey_meths(ENGINE *e, EVP_PKEY_METHOD **pmeth,
...
static int dstu_ciphers(ENGINE *e, const EVP_CIPHER **cipher,
                        const int **nids, int nid)
{
    if (cipher && nid) {
...
static int bind_dstu(ENGINE *e, const char *id)
{
    ...
    if (!EVP_PBE_alg_add_type
        (EVP_PBE_TYPE_PRF, NID_hmacWithDstu34311, -1, NID_dstu34311, NULL)) {
        DSTUerr(DSTU_F_BIND_DSTU, ERR_R_EVP_LIB);
        dstu_asn1_meth_finish();
        dstu_pkey_meth_finish();
        return 0;
