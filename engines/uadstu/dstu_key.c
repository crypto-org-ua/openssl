                                        ...
                                        int is_little_endian)
{
    ...
    /*
     * Checking if group represents a standard curve. If we get NID_undef,
     * that means the curve is custom
     */
                        ...
                        int is_little_endian)
{
    ...
    if (DSTU_STANDARD_CURVE == params->curve->type) {
        ...
        if (is_little_endian) {
            reverse_buffer =
                OPENSSL_malloc(ASN1_STRING_length
...
void DSTU_KEY_CTX_set(DSTU_KEY_CTX * ctx, EC_GROUP *group,
                      unsigned char *sbox)
{
    if (group) {
        if (ctx->group)
